...
if "%FFMPEG_PATH%"=="" set "FFMPEG_PATH=%~dp0tools\ffmpeg\ffmpeg-8.0.1-essentials_build\bin\ffmpeg.exe"
...
if "%TARGET_WIDTH%"=="" set "TARGET_WIDTH=1920"

set "TARGET_HEIGHT=%TARGET_HEIGHT%"
if "%TARGET_HEIGHT%"=="" set "TARGET_HEIGHT=-2"
...
if "%FFMPEG_WATCHDOG_IDLE_COUNT%"=="" set "FFMPEG_WATCHDOG_IDLE_COUNT=3"
set "FFMPEG_LOG=%~dp0logs\ffmpeg_watchdog.log"

:loop
echo [FFmpeg] Pulling %CAMERA_RTSP_URL% and publishing to %MEDIA_MTX_RTSP_URL%
if exist "%FFMPEG_LOG%" del /f /q "%FFMPEG_LOG%"
start "" /b cmd /c ""%FFMPEG_PATH%" -rtsp_transport tcp -rtsp_flags prefer_tcp -timeout 5000000 -fflags +discardcorrupt+nobuffer -flags low_delay -avioflags direct -max_delay 0 -reorder_queue_size 0 -analyzeduration 500000 -probesize 131072 -i "%CAMERA_RTSP_URL%" -an -vf "fps=%TARGET_FPS%,scale=%TARGET_WIDTH%:%TARGET_HEIGHT%" -c:v libx264 -preset ultrafast -tune zerolatency -profile:v baseline -level 4.1 -pix_fmt yuv420p -g %TARGET_FPS% -keyint_min %TARGET_FPS% -bf 0 -muxdelay 0 -muxpreload 0 -f rtsp -rtsp_transport tcp "%MEDIA_MTX_RTSP_URL%" > "%FFMPEG_LOG%" 2>&1"
