...
start /b "MediaMTX" cmd /c "cd /d %ROOT_DIR% && "%MEDIAMTX_EXE%" "%ROOT_DIR%mediamtx.yml" >> "%ROOT_DIR%logs\mediamtx.log" 2>>&1"

start /b "FFmpeg" cmd /c "cd /d %ROOT_DIR% && set ""FFMPEG_PATH=%FFMPEG_PATH%"" && set ""CAMERA_RTSP_URL=%CAMERA_RTSP_URL%"" && set ""MEDIA_MTX_RTSP_URL=%MEDIA_MTX_RTSP_URL%"" && .\ffmpeg-supervisor.cmd >> "%ROOT_DIR%logs\ffmpeg.log" 2>>&1"
